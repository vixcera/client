export const createStorage = async (key, token, url, id, status, exp) => {
    let index = 1;
    let currentKey = `${key}${index}`;
    const found = sessionStorage.getItem(currentKey);
    const expire = new Date().getTime() + exp * 60 * 1000
    if (!found) return sessionStorage.setItem(currentKey, JSON.stringify({ token, id, url, status, expire }));
    
    while (sessionStorage.getItem(currentKey)) {
        index++;
        currentKey = `${key}${index}`;
    }
    return sessionStorage.setItem(currentKey, JSON.stringify({ token, id, url, status, expire }));
};
...
export const allStorage = () => {
    ...
    while (i <= 10) {
        ...
        const defaultValues = { currentKey, token: null, url: null, id: null, status: null, expire: null };
